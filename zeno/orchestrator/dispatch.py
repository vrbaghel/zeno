...
from zeno.db.models import DbAgent, DbAgentAssignment, DbSession, DbTask
from zeno.memory.models import MemContext, MemLog, MemTrace, MemVault
from zeno.memory.retrieval import build_context
from zeno.orchestrator.errors import DispatchError, ParseError, StorageError, ValidationError

logger = logging.getLogger(__name__)


def _render_mem_context(ctx: MemContext) -> str:
    relevant = "\n\n".join([d.to_document().strip() for d in ctx.relevant_traces if d.to_document()])
    history = "\n\n".join([d.to_document().strip() for d in ctx.agent_logs if d.to_document()])
...
def _trace_from_adaptor_response(
    *,
    ...
    # been converted into AdaptorResponse payload + artifacts (log isn't part of
    # AdaptorResponse today). We store a minimal log summary.
...
async def dispatch_agent(
    *,
...
) -> tuple[AdaptorResponse, AdaptorMetrics, MemTrace]:
    try:
    ...
    try:
        trace = _trace_from_adaptor_response(
            response=response,
            vault=vault,
            session_id=session.id,
            task_id=task.id,
        ...
        raise ParseError("Failed to build memory trace from response", detail=str(e)) from e
