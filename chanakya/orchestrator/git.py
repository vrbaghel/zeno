...
from chanakya.orchestrator.errors import InitializationError, MergeError
...
async def create_worktree(
    ...
    worktree_path = root / ".chanakya" / "worktrees" / str(session_id) / str(task_id)
    worktree_path.parent.mkdir(parents=True, exist_ok=True)

    branch_name = f"chanakya/{session_id}/{task_id}"
...
async def merge_worktree(
    ...
    root = Path(working_directory).resolve()
    msg = f"chanakya: {task_title}".strip()
